...
$(document).ready(function() {
    ...
    $("nav.main a, .jumbotron.first a").bind("click",function(event){
     ...
     $("a.to-schedule").bind("click",function(event){
        event.preventDefault();
        $(this).blur();
         var target = $(this).attr("href");
         $("html, body").stop().animate({
    ...
    {    
        ...
        $sH2.each (function (i) 
        {
            ...
            if (($window.scrollTop () + 100) >= yPos[i]) 
                isActive = i;
                $('nav.navbar').addClass('top');
            ...
            if (isActive != null)
            {
                 ...
                 $('nav.main li a').eq(isActive).addClass('selected');
            }
            ...
            if($(window).scrollTop() + $(window).height() == $(document).height()) {
               $("nav.main li a").removeClass("selected");
               $('nav.main li.contact a').eq (0).addClass ('selected');
           }
        });
